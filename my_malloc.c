...
void* myallocate(int bytes, char * file, int line, int req){
	...
	if(myBlock == NULL) {
		...
		PageTable = (PageMetadata *) ((char *)threadNodeList - ((maxThreadPages + maxSwapPages) * sizeof(PageMetadata)));
	...
	//IF CALLED BY THREAD
		...
		if(threadNodeList[current_thread].firstPage == -1) {
			...
			if (reqPages > numLocalPagesLeft) {
				...
				int topPage = 0; // Next page to be evicted from main mem
				int swapPage = maxThreadPages; // Page to be thrown into main mem // Starts at first swapFile page
		...
		while (ourPage != -1) {
			...
			if (VMPage != ourPage && VMPage < maxThreadPages) {
